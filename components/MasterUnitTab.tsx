    ...
    <div className="space-y-6">
      <div className="bg-[#0B101E] rounded-3xl border border-slate-800/50 p-6 shadow-xl">
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-8">
          <div>
            <h2 className="text-xl font-bold text-slate-100 flex items-center gap-2">
              <MapPin className="w-5 h-5 text-emerald-400" /> Master Unit Kerja
            </h2>
            <p className="text-sm text-slate-400 mt-1">Kelola daftar unit staff yang muncul pada form kuesioner.</p>
          </div>
        ...
        {isAdding && (
          ...
          >
            <h4 className="text-sm font-bold text-emerald-400 mb-4">Tambah Unit Baru</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5">Nama Unit</label>
                <input 
                  type="text" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {list.map(pos => (
                  <div 
                    ...
                    className={`bg-[#020918]/60 border ${pos.is_active ? 'border-slate-800/80' : 'border-red-900/30 opacity-60'} rounded-xl p-4 flex flex-col justify-between transition-colors ${draggedId === pos.id ? 'opacity-50 border-emerald-500/50' : ''} cursor-move`}
                  >
                    {isEditing === pos.id ? (
